var jsPsychCanvasSliderResponseMulti = (function (jspsych) {
  'use strict';

  const info = {
      name: "canvas-slider-response-multi",
      parameters: {
  ...
  class CanvasSliderResponsePlugin {
      ...
      trial(display_element, trial) {
        var numOfSliders = trial.slider_number;
        let html = "";
        let exp = trial.struct_object;
        console.log(exp);
        slider_prompts = exp.getTopDiagnoses();
        for (var x = 0; x < numOfSliders;x++)
        {
          ...
          display_element
              ...
              var sliders = display_element.getElementsByClassName("jspsych-canvas-slider-response-response");
              var l = sliders.length;
              response.response = [];
              for (i = 0; i < l; i++)
              {
                response.response.push(sliders[x].valueAsNumber)
              }
